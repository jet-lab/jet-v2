...
const UserGuidePage = ({
    ...
    isFirstPage,
    isEndPage
}: IUserGuidePage) => <div className="flex flex-col">
        <div className='flex'>
            <div className="flex flex-col flex-1">
                <p className="font-light my-2 text-4xl">{currentPage}/{totalPages}</p>
                <p className="font-normal my-2 text-2xl">{headline}</p>
                {content}
            </div>
            <div className="flex flex-1">swap picture</div>
        </div>
        <div className="flex font-normal items-center text 2xl my-2">
            <a
            ...
            {!isFirstPage && <button className="flex-1" onClick={previousPage}>
                &#8592; Previous
            </button>}
            {!isEndPage && <button className="flex-1" onClick={nextPage}>
                Next &#8594;
            </button>}
            {dismiss && <button className="flex-1" onClick={dismiss}>
                Get Started &#8594;
            </button>}
        </div>
    </div>

const Page1Content = () => <p className="font-normal my-2 text-base">
...
const Page2Content = () => <p className="font-normal text-base">
    Interest rates in a fixed rate market are determined by lenders and borrowers who are transacting as
    makers by <b className="font-bold">offering loans</b> and <b className="font-bold">requesting loans</b>. Each loan offer has a fixed rate chosen
    by the lender and each loan request has has a fixed rate chosen by the borrowers.
    <br />
    Borrowers seeking immediate liquidity may choose to <b className="font-bold">borrow now</b> by accepting loans on offer
    which will determine in advance the interest rate for a loan of whatever size they choose. Lenders
    may choose to <b className="font-bold">lend now</b> by satisfying loan requests on the book. The interest rate will be
    determined in advance by the rate associated with the requests that are filled.
</p>

const Page3Content = () => <p className="font-normal text-base">
...
const Page4Content = () => <p className="font-normal text-base">
    ...
    maturity by using the <b className='font-bold'>auto roll</b> feature.
</p>


export const UserGuide = () =>    // <div className="relative top-44 left-96 rounded">
    <DismissModal storageKey='fixed-term-guide' title="Fixed Rate Debt Markets" className='w-3/4'>
        {({ dismiss }) => <Carousel
            ...
            pages={({ previousPage, nextPage, pageNumber, isEndPage, isFirstPage }) => [
                <UserGuidePage key={1} isEndPage={isEndPage} isFirstPage={isFirstPage} headline="Select a market" nextPage={nextPage} previousPage={previousPage} currentPage={pageNumber} totalPages={4} content={<Page1Content />} />,
                <UserGuidePage key={2} isEndPage={isEndPage} isFirstPage={isFirstPage} headline="Transact as a maker or taker" nextPage={nextPage} previousPage={previousPage} currentPage={pageNumber} totalPages={4} content={<Page2Content />} />,
                <UserGuidePage key={3} isEndPage={isEndPage} isFirstPage={isFirstPage} headline="Submit and order" nextPage={nextPage} previousPage={previousPage} currentPage={pageNumber} totalPages={4} content={<Page3Content />} />,
                <UserGuidePage dismiss={dismiss} key={4} isEndPage={isEndPage} isFirstPage={isFirstPage} headline="Submit and order" nextPage={nextPage} previousPage={previousPage} currentPage={pageNumber} totalPages={4} content={<Page4Content />} />,
            ]}
        />}
    </DismissModal>
