import { AssociatedToken, FixedTermMarket, MarginAccount, MarketAndconfig, Pool, redeem, repay, settle, TokenAmount } from "@jet-lab/margin";
...
export const getOwedTokens = async (
    ...
    const marginUser = await market.market.fetchMarginUser(marginAccount)

    const owedTokens = marginUser?.assets.entitledTokens || new BN(0)
    try {
