...
class Number128 {
    ...
    toNumber() {
        return (0, token_1.bnToNumber)(this._bn) / 10 ** Number128.PRECISION;
    }
    ...
    toBn(exponent) {
        ...
        let precValue = Number128.tenPow(new anchor_1.BN(Math.abs(extraPrecision)));
        if (extraPrecision < 0) {
    ...
    static fromBits(bits) {
        return new Number128(new anchor_1.BN(bits, "le"));
    }
    static from(bn) {
    ...
    static min(a, b) {
        return new Number128(anchor_1.BN.min(a._bn, b._bn));
    }
    static max(a, b) {
        return new Number128(anchor_1.BN.max(a._bn, b._bn));
    }
    static tenPow(exponent) {
        return new anchor_1.BN(10).pow(exponent);
    }
}
exports.Number128 = Number128;
Number128.PRECISION = 10;
Number128.ONE = new Number128(new anchor_1.BN(10000000000));
Number128.ZERO = new Number128(new anchor_1.BN(0));
Number128.MAX = new Number128(new anchor_1.BN("340282366920938463463374607431768211455"));
Number128.U64_MAX = new anchor_1.BN("18446744073709551615");
