...
exports.Pool = exports.feesBuffer = void 0;
...
const poolTokenChange_1 = require("./poolTokenChange");
const pda_1 = require("../../utils/pda");
const utils_1 = require("../../utils");
const positionTokenMetadata_1 = require("../positionTokenMetadata");
exports.feesBuffer = web3_js_1.LAMPORTS_PER_SOL * 0.075;
/**
...
class Pool {
     ...
     * @param tokenConfig
     */
    constructor(programs, addresses, tokenConfig) {
        this.programs = programs;
        this.addresses = addresses;
        this.tokenConfig = tokenConfig;
        this.depositNoteMetadata = new positionTokenMetadata_1.PositionTokenMetadata({ programs, tokenMint: addresses.depositNoteMint });
        this.loanNoteMetadata = new positionTokenMetadata_1.PositionTokenMetadata({ programs, tokenMint: addresses.loanNoteMint });
        const zero = new anchor_1.BN(0);
        this.prices = {
            priceValue: utils_1.Number192.ZERO,
     ...
     */
    get vaultRaw() {
        return utils_1.Number192.fromDecimal(this.info?.vault.amount.lamports ?? new anchor_1.BN(0), 0);
    }
    /**
    ...
    get borrowedTokensRaw() {
        if (!this.info) {
            return utils_1.Number192.ZERO;
        }
        return utils_1.Number192.fromBits(this.info.marginPool.borrowedTokens);
    }
    /**
     ...
     * @memberof Pool
     */
    get uncollectedFeesRaw() {
        if (!this.info) {
            return utils_1.Number192.ZERO;
        }
        return utils_1.Number192.fromBits(this.info.marginPool.uncollectedFees);
    }
    /**
    ...
    get depositNotePrice() {
        return {
            value: this.prices.depositNotePrice,
            exponent: this.info?.tokenPriceOracle.exponent ?? 0,
            timestamp: (0, token_1.bigIntToBn)(this.info?.tokenPriceOracle.timestamp),
            isValid: Number(this.info ? this.info.tokenPriceOracle.status === client_1.PriceStatus.Trading : false)
        };
    }
    get loanNotePrice() {
        return {
    ...
    async refresh() {
        const [marginPoolInfo, poolTokenMintInfo, vaultMintInfo, depositNoteMintInfo, loanNoteMintInfo, tokenMetadataInfo, depositNoteMetadataInfo, loanNoteMetadataInfo] = await this.programs.marginPool.provider.connection.getMultipleAccountsInfo([
        ...
        if (!marginPoolInfo ||
            ...
            const marginPool = this.programs.marginPool.coder.accounts.decode("marginPool", marginPoolInfo.data);
            const tokenMint = token_1.AssociatedToken.decodeMint(poolTokenMintInfo, this.addresses.tokenMint);
            ...
            this.info = {
                marginPool,
                tokenMint,
                vault: token_1.AssociatedToken.decodeAccount(vaultMintInfo, this.addresses.vault, tokenMint.decimals),
                depositNoteMint: token_1.AssociatedToken.decodeMint(depositNoteMintInfo, this.addresses.depositNoteMint),
                loanNoteMint: token_1.AssociatedToken.decodeMint(loanNoteMintInfo, this.addresses.loanNoteMint),
                tokenPriceOracle: (0, client_1.parsePriceData)(oracleInfo.data),
                tokenMetadata: this.programs.metadata.coder.accounts.decode("tokenMetadata", tokenMetadataInfo.data)
            };
        }
     ...
     ****************************/
    calculatePrices(pythPrice) {
        if (!pythPrice ||
            pythPrice.status !== client_1.PriceStatus.Trading ||
        ...
        const priceValue = utils_1.Number192.fromDecimal((0, token_1.bigIntToBn)(pythPrice.aggregate.priceComponent), pythPrice.exponent);
        const confValue = utils_1.Number192.fromDecimal((0, token_1.bigIntToBn)(pythPrice.aggregate.confidenceComponent), pythPrice.exponent);
        const twapValue = utils_1.Number192.fromDecimal((0, token_1.bigIntToBn)(pythPrice.emaPrice.valueComponent), pythPrice.exponent);
    ...
    depositNoteExchangeRate() {
        ...
        const depositNotes = anchor_1.BN.max(new anchor_1.BN(1), this.info.marginPool.depositNotes);
        const totalValue = utils_1.Number192.max(utils_1.Number192.ONE, this.totalValueRaw);
        return totalValue.sub(this.uncollectedFeesRaw).div(utils_1.Number192.from(depositNotes));
    }
    /**
    ...
    loanNoteExchangeRate() {
        ...
        const loanNotes = anchor_1.BN.max(new anchor_1.BN(1), this.info.marginPool.loanNotes);
        const totalBorrowed = utils_1.Number192.max(utils_1.Number192.ONE, this.borrowedTokensRaw);
        return totalBorrowed.div(utils_1.Number192.from(loanNotes));
    }
    /**
     ...
     * @returns {number}
     */
    static getCcRate(reserveConfig, utilRate) {
     ...
     * @memberof Pool
     */
    static getDepositRate(ccRate, utilRatio, feeFraction) {
     ...
     * @memberof Pool
     */
    async marginRefreshAllPositionPrices({ pools, marginAccount }) {
        const instructions = [];
     ...
     * @memberof Pool
     */
    async marginRefreshPositionPrice(marginAccount) {
        const instructions = [];
     ...
     * @memberof Pool
     */
    async withMarginRefreshAllPositionPrices({ instructions, pools, marginAccount }) {
        for (const pool of Object.values(pools)) {
            const positionRegistered = !!marginAccount.getPositionNullable(pool.addresses.depositNoteMint) ||
     ...
     * @memberof Pool
     */
    async withMarginRefreshPositionPrice({ instructions, marginAccount }) {
        if (!marginAccount || !this.info)
            throw new Error("Margin or pool not fully setup");
     ...
     */
    async deposit({ marginAccount, change, source = token_1.TokenFormat.unwrappedSol }) {
        (0, assert_1.default)(marginAccount);
        (0, assert_1.default)(change);
        const instructions = [];
     ...
     * @memberof Pool
     */
    async withDeposit({ instructions, marginAccount, source = token_1.TokenFormat.unwrappedSol, change }) {
        ...
        const wrappedSource = await token_1.AssociatedToken.withBeginTransferFromSource({
            instructions,
            provider,
            mint,
            feesBuffer: exports.feesBuffer,
            source
        });
    ...
    async marginBorrow({ marginAccount, pools, change, destination }) {
        ...
        if (destination !== undefined) {
            ...
            const poolPosition = Object.values(marginAccount.poolPositions).find(position => position.pool && position.pool.address.equals(this.address));
            if (!poolPosition)
                throw new Error("Attempting to withdraw after borrowing, but can not find the pool position in the margin account to calculate the withdraw amount.");
            const previousDepositAmount = poolPosition.depositBalance;
            const withdrawChange = previousDepositAmount.tokens > 0 ? poolTokenChange_1.PoolTokenChange.shiftBy(change.value) : poolTokenChange_1.PoolTokenChange.setTo(0);
        ...
        return await marginAccount.sendAll([(0, utils_1.chunks)(11, refreshInstructions), registerinstructions, instructions]);
    }
    ...
    async withMarginBorrow({ instructions, marginAccount, change }) {
        (0, assert_1.default)(instructions);
        (0, assert_1.default)(marginAccount);
        (0, assert_1.default)(change);
    ...
    async marginRepay({ marginAccount, pools, source, change, closeLoan, signer }) {
        await marginAccount.refresh();
        const refreshInstructions = [];
        const instructions = [];
        ...
        (0, assert_1.default)(depositPosition);
        await this.withMarginRefreshAllPositionPrices({
            instructions: refreshInstructions,
            pools: Object.values(pools),
            marginAccount
        ...
        return await marginAccount.sendAll([(0, utils_1.chunks)(11, refreshInstructions), instructions]);
    }
    async withMarginRepay({ instructions, marginAccount, change }) {
        const depositAccount = this.findDepositPositionAddress(marginAccount);
        const loanAccount = this.findLoanPositionAddress(marginAccount);
        await marginAccount.withAdapterInvoke({
            instructions,
    ...
    async withRepay({ instructions, marginAccount, source, change, feesBuffer, sourceAuthority }) {
        const wrappedSource = await token_1.AssociatedToken.withBeginTransferFromSource({
            instructions,
    ...
    async withdraw({ marginAccount, pools, change, destination = token_1.TokenFormat.unwrappedSol }) {
        const refreshInstructions = [];
        const instructions = [];
        await this.withMarginRefreshAllPositionPrices({
            instructions: refreshInstructions,
    ...
    async withWithdraw({ instructions, marginAccount, destination = token_1.TokenFormat.unwrappedSol, change }) {
        const provider = marginAccount.provider;
        const mint = this.tokenMint;
        const source = marginAccount.getPositionNullable(this.addresses.depositNoteMint)?.address;
        (0, assert_1.default)(source, "No deposit position");
        const withdrawDestination = await token_1.AssociatedToken.withBeginTransferToDestination({
            instructions,
            provider,
            mint,
            destination
     ...
     */
    async splTokenSwap({ marginAccount, pools, outputToken, swapPool, swapAmount, minAmountOut, repayWithOutput }) {
        (0, assert_1.default)(marginAccount);
        (0, assert_1.default)(swapAmount);
        const refreshInstructions = [];
        const registerInstructions = [];
        ...
        const projectedRiskLevel = this.projectAfterMarginSwap(marginAccount, swapAmount.tokens, minAmountOut.tokens, outputToken, true).riskIndicator;
        ...
        const transitSourceAccount = await token_1.AssociatedToken.withCreate(transitInstructions, marginAccount.provider, marginAccount.address, this.addresses.tokenMint);
        // Transit destination account fetch / creation
        const transitDestinationAccount = await token_1.AssociatedToken.withCreate(transitInstructions, marginAccount.provider, marginAccount.address, outputToken.tokenMint);
        // Default change kind to a shiftBy
        const accountPoolPosition = marginAccount.poolPositions[this.symbol];
        let changeKind = poolTokenChange_1.PoolTokenChange.shiftBy(swapAmount);
        // If swapping total balance, use setTo(0)
        if (swapAmount.gte(accountPoolPosition.depositBalance)) {
            changeKind = poolTokenChange_1.PoolTokenChange.setTo(0);
        }
        ...
        if (swapAmount.gt(accountPoolPosition.depositBalance) && marginAccount.pools) {
            ...
            await this.withMarginBorrow({
                instructions: transitInstructions,
                marginAccount,
                change: poolTokenChange_1.PoolTokenChange.setTo(accountPoolPosition.loanBalance.add(difference))
            });
        }
        // Swap ix
        ...
        if (!outputDebtPosition.isZero() && repayWithOutput) {
            const change = minAmountOut.gt(outputDebtPosition)
                ? poolTokenChange_1.PoolTokenChange.setTo(0)
                : poolTokenChange_1.PoolTokenChange.shiftBy(minAmountOut);
    ...
    async withSPLTokenSwap({ instructions, marginAccount, outputToken, swapPool, changeKind, minAmountOut, sourceAccount, destinationAccount, transitSourceAccount, transitDestinationAccount }) {
        ...
        await marginAccount.withAdapterInvoke({
            ...
            adapterMetadata: (0, pda_1.findDerivedAccount)(this.programs.config.metadataProgramId, this.programs.config.marginSwapProgramId),
            adapterInstruction: await this.programs.marginSwap.methods
                ...
                swapInfo: {
                    ...
                    authority: (0, pda_1.findDerivedAccount)(new web3_js_1.PublicKey(swapPool.swapProgram), swapPool.swapPool),
     ...
     */
    findDepositPositionAddress(marginAccount) {
        return marginAccount.findPositionTokenAddress(this.addresses.depositNoteMint);
    }
    async withGetOrRegisterDepositPosition({ instructions, marginAccount }) {
     ...
     * @memberof MarginAccount
     */
    async withRegisterDepositPosition({ instructions, marginAccount }) {
        const positionTokenMint = this.addresses.depositNoteMint;
    ...
    async withCloseDepositPosition({ instructions, marginAccount }) {
    ...
    async withdrawAndCloseDepositPosition({ marginAccount, destination }) {
        await marginAccount.refresh();
        const position = marginAccount.getPositionNullable(this.addresses.depositNoteMint);
        if (position) {
            const instructions = [];
            if (position.balance.gt(new anchor_1.BN(0))) {
                const destinationAddress = (0, anchor_1.translateAddress)(destination);
                const isDestinationNative = token_1.AssociatedToken.isNative(marginAccount.owner, this.tokenMint, destinationAddress);
                let marginWithdrawDestination;
                if (!isDestinationNative) {
                    marginWithdrawDestination = destinationAddress;
                }
                else {
                    marginWithdrawDestination = token_1.AssociatedToken.derive(this.tokenMint, marginAccount.owner);
                }
                await marginAccount.withUpdatePositionBalance({ instructions, position: position.address });
                await token_1.AssociatedToken.withCreate(instructions, marginAccount.provider, marginAccount.owner, this.tokenMint);
                await this.withWithdraw({
                    instructions,
                    marginAccount: marginAccount,
                    destination: marginWithdrawDestination,
                    change: poolTokenChange_1.PoolTokenChange.setTo(0)
                });
                if (isDestinationNative) {
                    token_1.AssociatedToken.withClose(instructions, marginAccount.owner, this.tokenMint, destinationAddress);
                }
            }
    ...
    findLoanPositionAddress(marginAccount) {
        return (0, pda_1.findDerivedAccount)(this.programs.config.marginPoolProgramId, marginAccount.address, this.addresses.loanNoteMint);
    }
    async withGetOrRegisterLoanPosition({ instructions, marginAccount }) {
    ...
    async withRegisterLoanPosition(instructions, marginAccount) {
    ...
    async withCloseLoanPosition({ instructions, marginAccount }) {
        const loanNoteAccount = (0, pda_1.findDerivedAccount)(this.programs.config.marginPoolProgramId, marginAccount.address, this.addresses.loanNoteMint);
        await marginAccount.withAdapterInvoke({
            instructions,
            adapterProgram: this.programs.config.marginPoolProgramId,
            adapterMetadata: this.addresses.marginPoolAdapterMetadata,
    ...
    projectAfterAction(marginAccount, amount, action, 
    ...
    projectAfterDeposit(marginAccount, amount) {
        ...
        const amountValue = utils_1.Number128.from((0, token_1.numberToBn)(amount * this.prices.priceValue.toNumber()));
    ...
    projectAfterWithdraw(marginAccount, amount) {
        ...
        const weightedCollateral = marginAccount.valuation.weightedCollateral
            ...
            .toNumber();
        const liabilities = marginAccount.valuation.liabilities.toNumber();
        const riskIndicator = marginAccount.computeRiskIndicator(requiredCollateral, weightedCollateral > 0 ? weightedCollateral : 0, // ok (but weird!) - guarded by G1
        liabilities);
        return { riskIndicator, depositRate, borrowRate };
    }
    /// Projects the deposit / borrow rates and user's risk level after a borrow from the pool.
    projectAfterBorrow(marginAccount, amount) {
        if (this.info === undefined) {
            return this.getDefaultPoolProjection(marginAccount);
        }
    ...
    projectAfterRepay(marginAccount, amount) {
        const position = marginAccount.poolPositions[this.symbol];
        ...
        const riskIndicator = marginAccount.computeRiskIndicator(requiredCollateral >= 0 ? requiredCollateral : 0, // ok - guarded by G1
        weightedCollateral, liabilities >= 0 ? liabilities : 0 // ok - guarded by G1
        );
        return { riskIndicator, depositRate, borrowRate };
    }
    /// Projects the deposit / borrow rates and user's risk level after repaying a loan from the pool.
    projectAfterRepayFromDeposit(marginAccount, amount) {
        const position = marginAccount.poolPositions[this.symbol];
    ...
    /// Projects the deposit / borrow rates and user's risk level after a borrow from the pool.
    projectAfterBorrowAndNotWithdraw(marginAccount, amount) {
        if (this.info === undefined) {
            return this.getDefaultPoolProjection(marginAccount);
        }
        const borrowedTokens = this.borrowedTokens.tokens + amount;
    ...
    projectAfterMarginSwap(marginAccount, amount, minAmountOut, outputToken, setupCheck) {
        ...
        const inputRequiredCollateralFactor = this.loanNoteMetadata.valueModifier.toNumber() *
            (setupCheck ? marginAccount_1.MarginAccount.SETUP_LEVERAGE_FRACTION.toNumber() : 1);
        const inputTokenAssetValue = inputDepositBalance * inputTokenPrice;
        const outputRequiredCollateralFactor = outputToken.loanNoteMetadata.valueModifier.toNumber() *
            (setupCheck ? marginAccount_1.MarginAccount.SETUP_LEVERAGE_FRACTION.toNumber() : 1);
        ...
        const riskIndicator = (totalLiabilities +
            requiredCollateral -
    ...
    getDefaultPoolProjection(marginAccount) {
        return {
 ...
 * @returns {number}
 */
Pool.interpolate = (x, x0, x1, y0, y1) => {
    (0, assert_1.default)(x >= x0);
    (0, assert_1.default)(x <= x1);
