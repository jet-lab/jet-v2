...
exports.numberToBigInt = exports.bigIntToNumber = exports.bnToBigInt = exports.bigIntToBn = exports.bnToNumber = exports.numberToBn = exports.AssociatedToken = exports.TokenFormat = void 0;
...
class AssociatedToken {
    ...
    static async loadMultipleOrNative({ connection, owner, mints, decimals }) {
        ...
        if (mints) {
            addresses = [];
            const mintAddresses = mints.map(anchor_1.translateAddress);
            ...
            const { value } = await connection.getTokenAccountsByOwner(ownerAddress, { programId: token_instructions_1.TOKEN_PROGRAM_ID });
            accountInfos = value.map(acc => acc.account);
            addresses = value.map(acc => acc.pubkey);
            mints = accountInfos.map(acc => (acc && spl_token_1.AccountLayout.decode(acc.data).mint) ?? web3_js_1.PublicKey.default);
            ...
            addresses.push(ownerAddress);
            mints.push(spl_token_1.NATIVE_MINT);
        }
        if (decimals === undefined) {
            decimals = [];
            const mintInfos = await AssociatedToken.loadMultipleAccountsInfoBatched(connection, mints.map(anchor_1.translateAddress));
            for (let i = 0; i < mintInfos.length; i++) {
                const mintInfo = mintInfos[i];
                if ((0, anchor_1.translateAddress)(mints[i]).equals(spl_token_1.NATIVE_MINT)) {
                    decimals.push(this.NATIVE_DECIMALS);
                }
                else if ((0, anchor_1.translateAddress)(mints[i]).equals(web3_js_1.PublicKey.default)) {
                    decimals.push(0);
                }
                    ...
                    const mint = spl_token_1.MintLayout.decode(mintInfo.data);
        ...
        const accounts = [];
        for (let i = 0; i < mints.length; i++) {
            const mint = (0, anchor_1.translateAddress)(mints[i]);
            ...
            const isNative = mint.equals(spl_token_1.NATIVE_MINT) && address.equals(ownerAddress);
    ...
    static async loadMultipleAux({ connection, addresses, decimals }) {
        ...
        if (decimals === undefined) {
            ...
            const mintPubkeys = accountInfos.map(acc => {
                return (acc && spl_token_1.AccountLayout.decode(acc.data).mint) ?? web3_js_1.PublicKey.default;
            });
            const mintInfos = await AssociatedToken.loadMultipleAccountsInfoBatched(connection, mintPubkeys);
            for (let i = 0; i < mintInfos.length; i++) {
                const mintInfo = mintInfos[i];
                if (mintPubkeys[i].equals(web3_js_1.PublicKey.default)) {
                    decimals.push(0);
                }
                else if (mintInfo === null) {
                    decimals.push(0);
    ...
    static async loadMultipleAccountsInfoBatched(connection, publicKeys, batchSize = 100) {
        const batches = (0, utils_1.chunks)(batchSize, publicKeys);
    ...
    static decodeAccount(data, address, decimals) {
        ...
        if (data && !data.owner.equals(token_instructions_1.TOKEN_PROGRAM_ID))
            throw new spl_token_1.TokenInvalidAccountOwnerError();
        if (data && data.data.length != spl_token_1.ACCOUNT_SIZE)
            throw new spl_token_1.TokenInvalidAccountSizeError();
        ...
        return new AssociatedToken(publicKey, info, tokenAmount_1.TokenAmount.account(info, decimals));
    }
    /**
    ...
    static decodeNative(info, address) {
        ...
        if (info && info.data.length != 0)
            throw new spl_token_1.TokenInvalidAccountSizeError();
        return new AssociatedToken(publicKey, null, tokenAmount_1.TokenAmount.lamports(new anchor_1.BN(info?.lamports.toString() ?? "0"), this.NATIVE_DECIMALS));
    }
    /**
    ...
    static isNative(owner, mint, tokenAccountOrNative) {
        ...
        return mintPubkey.equals(spl_token_1.NATIVE_MINT) && tokenAccountOrNativePubkey.equals(ownerPubkey);
    }
    /**
    ...
    static async withCreate(instructions, provider, owner, mint) {
        ...
        if (!(await AssociatedToken.exists(provider.connection, mintAddress, ownerAddress))) {
            const ix = (0, spl_token_1.createAssociatedTokenAccountInstruction)(provider.wallet.publicKey, tokenAddress, ownerAddress, mintAddress);
    ...
    static withClose(instructions, owner, mint, rentDestination) {
        const ownerPubkey = (0, anchor_1.translateAddress)(owner);
        const mintPubkey = (0, anchor_1.translateAddress)(mint);
        const rentDestinationPubkey = (0, anchor_1.translateAddress)(rentDestination);
        const tokenAddress = this.derive(mintPubkey, ownerPubkey);
        const ix = (0, spl_token_1.createCloseAccountInstruction)(tokenAddress, rentDestinationPubkey, ownerPubkey);
        instructions.push(ix);
    }
    ...
    static async withWrapNative(instructions, provider, feesBuffer) {
        const owner = (0, anchor_1.translateAddress)(provider.wallet.publicKey);
        ...
        const associatedToken = await this.withCreate(instructions, provider, owner, spl_token_1.NATIVE_MINT);
        ...
        const syncNativeIX = (0, spl_token_1.createSyncNativeInstruction)(associatedToken);
    ...
    static withUnwrapNative(instructions, owner) {
        ...
        this.withClose(instructions, owner, spl_token_1.NATIVE_MINT, owner);
    }
    /** Add wrap SOL IX
     * @param instructions
     * @param provider
...
function numberToBn(number) {
    return new anchor_1.BN(numberToBigInt(number).toString());
}
