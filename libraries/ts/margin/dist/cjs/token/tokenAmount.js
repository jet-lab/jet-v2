...
class TokenAmount {
    ...
    constructor(lamports, decimals) {
        if (!anchor_1.BN.isBN(lamports)) {
            console.warn("Amount is not a BN", lamports);
            lamports = new anchor_1.BN(0);
        }
    ...
    static zero(decimals) {
        return new TokenAmount(new anchor_1.BN(0), decimals);
    }
    /**
    ...
    static account(tokenAccount, decimals) {
        return new TokenAmount((0, associatedToken_1.bigIntToBn)(tokenAccount.amount), decimals);
    }
    /**
     * @static
     * @param {MintInfo} mint
     ...
     */
    static mint(mint) {
        return new TokenAmount((0, associatedToken_1.bigIntToBn)(mint.supply), mint.decimals);
    }
    /**
     * @static
