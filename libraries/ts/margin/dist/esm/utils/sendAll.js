 ...
 */
export async function sendAndConfirm(provider, instructions, signers, opts) {
    if (opts === undefined) {
        opts = provider.opts;
    }
    const { blockhash, lastValidBlockHeight } = await provider.connection.getLatestBlockhash(opts.preflightCommitment);
    const transaction = new Transaction({ feePayer: provider.wallet.publicKey, blockhash, lastValidBlockHeight }).add(...instructions);
 ...
 */
export async function sendAll(provider, transactions, opts) {
    if (opts === undefined) {
        opts = provider.opts;
    }
    const { blockhash, lastValidBlockHeight } = await provider.connection.getLatestBlockhash(opts.preflightCommitment);
