...
import { AssociatedToken, bigIntToBn, numberToBn, TokenFormat } from "../../token";
...
export const feesBuffer = LAMPORTS_PER_SOL * 0.075;
/**
...
export class Pool {
     ...
     */
    async deposit({ marginAccount, change, source = TokenFormat.unwrappedSol }) {
     ...
     * @memberof Pool
     */
    async withDeposit({ instructions, marginAccount, source = TokenFormat.unwrappedSol, change }) {
    ...
    async marginBorrow({ marginAccount, pools, change, destination }) {
        ...
        if (destination !== undefined) {
            ...
            const withdrawChange = previousDepositAmount.tokens > 0 ? PoolTokenChange.shiftBy(change.value) : PoolTokenChange.setTo(0);
    ...
    async withdraw({ marginAccount, pools, change, destination = TokenFormat.unwrappedSol }) {
        const refreshInstructions = [];
        const instructions = [];
        await this.withMarginRefreshAllPositionPrices({
            instructions: refreshInstructions,
    ...
    async withWithdraw({ instructions, marginAccount, destination = TokenFormat.unwrappedSol, change }) {
        const provider = marginAccount.provider;
        const mint = this.tokenMint;
    ...
    async splTokenSwap({ marginAccount, pools, outputToken, swapPool, swapAmount, minAmountOut, repayWithOutput }) {
        ...
        const transitSourceAccount = await AssociatedToken.withCreate(transitInstructions, marginAccount.provider, marginAccount.address, this.addresses.tokenMint);
        // Transit destination account fetch / creation
        const transitDestinationAccount = await AssociatedToken.withCreate(transitInstructions, marginAccount.provider, marginAccount.address, outputToken.tokenMint);
    ...
    async withSPLTokenSwap({ instructions, marginAccount, outputToken, swapPool, changeKind, minAmountOut, sourceAccount, destinationAccount, transitSourceAccount, transitDestinationAccount }) {
        ...
        await marginAccount.withAdapterInvoke({
            ...
            adapterMetadata: findDerivedAccount(this.programs.config.metadataProgramId, this.programs.config.marginSwapProgramId),
    ...
    findLoanPositionAddress(marginAccount) {
        return findDerivedAccount(this.programs.config.marginPoolProgramId, marginAccount.address, this.addresses.loanNoteMint);
    }
    ...
    async withCloseLoanPosition({ instructions, marginAccount }) {
        const loanNoteAccount = findDerivedAccount(this.programs.config.marginPoolProgramId, marginAccount.address, this.addresses.loanNoteMint);
        await marginAccount.withAdapterInvoke({
            instructions,
            adapterProgram: this.programs.config.marginPoolProgramId,
            adapterMetadata: this.addresses.marginPoolAdapterMetadata,
