...
pub async fn get_governance_and_realm(
    ...
    let governance = get_borsh_account::<GovernanceV2>(client.rpc.clone(), address).await?;

    get_borsh_account::<RealmV2>(client.rpc.clone(), &governance.realm)
        .await
        .map(|r| (governance, r))
}
...
pub async fn inspect_proposal_instructions(
    ...
    let proposal: ProposalV2 = get_borsh_account(client.rpc.clone(), &proposal_address)
        .await
        .with_context(|| "getting proposal")?;
    let mut anchor_parser = AnchorParser::new(client.rpc.clone());
    ...
    for (opt_index, option) in proposal.options.iter().enumerate() {
        ...
        for tx_index in 0..option.transactions_next_index {
            ...
            if let Some(proposal_tx) =
                get_prosposal_transaction(client.rpc.clone(), &tx_address).await?
            {
...
async fn try_parse_instruction(
    client: &Client,
    anchor_parser: &mut AnchorParser,
