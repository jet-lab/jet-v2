...
impl TransactionBuilder {
    ...
    pub fn encode(&self) -> Result<String, bincode::Error> {
        let mut compiled = Transaction::new_unsigned(Message::new(&self.instructions, None));
...
pub fn condense(txs: &[TransactionBuilder]) -> Result<Vec<TransactionBuilder>, bincode::Error> {
    ...
    loop {
        ...
        let next = find_first_condensed(&shrink_me)?;
...
fn find_first_condensed(txs: &[TransactionBuilder]) -> Result<usize, bincode::Error> {
    ...
    loop {
        ...
        let size = txs[0..try_len].ijoin().encode()?.len() + 64;
        if size > MAX_TX_SIZE {
