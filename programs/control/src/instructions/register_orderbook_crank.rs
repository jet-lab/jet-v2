...
#[derive(Accounts)]
pub struct RegisterOrderbookCrank<'info> {
...
pub fn register_orderbook_crank_handler(ctx: Context<RegisterOrderbookCrank>) -> Result<()> {
    let authority = [&ctx.accounts.authority.seed[..]];

    let mut data = vec![];
    let metadata = CrankMetadata {
