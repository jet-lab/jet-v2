...
impl<'info, T: ZeroCopy + Owner> AdapterLoader<'info, T> {
    ...
    pub fn load_adapter(acc_info: &AccountInfo<'info>) -> Result<EventQueue<'info>> {
        ...
        EventQueue::new_adapter(acc_info.data.clone())
    }
}
...
/// }
///
/// let seeds: &[&[u8]] = auto_stake::seeds(auto_stake::Seeds {
...
macro_rules! seeds {
    ($(
        ...
        pub mod $name {
            ...
            pub fn seeds<'a>(params: Seeds<'a>) -> Vec<&'a [u8]> {
                vec![
                    $($value,)+
                    $(params.$param,)+
                ]
...
use crate::orderbook::state::event_queue::{EventAdapterMetadata, EventQueue};
