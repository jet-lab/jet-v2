...
pub struct BondManager {
    ...
    pub asks_slab: Pubkey,
    /// The orderbook bids byteslab
    pub bids_slab: Pubkey,
...
impl BondManager {
    ...
    pub fn convert_tickets(&self, ticket_amount: u64) -> Result<u64> {
        match self.conversion_factor {
            decimals if decimals > 0 => ticket_amount.safe_mul(10u64.pow(decimals as u32)),
    ...
    pub fn convert_tokens(&self, token_amount: u64) -> Result<u64> {
        match self.conversion_factor {
            decimals if decimals > 0 => token_amount.safe_div(10u64.pow(decimals as u32)),
            decimals if decimals < 0 => {
...
impl Serialize for BondManager {
    ...
    where
        S: Serializer,
    {
        let mut s = serializer.serialize_struct("BondManager", 8)?;
