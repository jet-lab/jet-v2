...
#[derive(Accounts)]
pub struct SplSwapPoolBalance<'info> {
    payer: Signer<'info>,
...
pub fn spl_swap_pool_balance_handler(ctx: Context<SplSwapPoolBalance>) -> Result<()> {
    let SplSwapPoolBalance {
        mint_a,
        mint_b,
        scratch_a,
        scratch_b,
...
fn apply_change<'info>(
    ctx: &Context<SplSwapPoolBalance<'info>>,
    change: Number128,
    price: Number128,
...
fn read_price(pyth_price: &AccountInfo) -> Number128 {
    ...
    let price_value = price_result.get_current_price_unchecked();
