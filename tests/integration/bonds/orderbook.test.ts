...
describe("jet-bonds", async () => {
  ...
  it("alice mints bond tickets", async () => {
    ...
    assert(
      resultingTickets.toString() ===
        new BN(
          TOKENS_EXCHANGED.toNumber() * 10 ** bondMarket.info.conversionFactor
        ).toString()
    );
  });
  ...
  it("alice stakes some tickets", async () => {
    ...
    assert(
      resultingTickets.toString() ===
        TOKENS_EXCHANGED.mul(new BN(10 ** bondMarket.info.conversionFactor))
          .sub(STAKE_AMOUNT)
          .toString()
    );
    assert(
      claimTicket.redeemable.toString() ===
        STAKE_AMOUNT.div(
          new BN(10 ** bondMarket.info.conversionFactor)
        ).toString()
    );
  });
  ...
  it("loads orderbook and asserts borrow order", async () => {
    ...
    assert(
      new PublicKey(order.account_key).toString() === alice.key.toString()
    );
    assert(
