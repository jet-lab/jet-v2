...
export class Transactor {
  ...
  async signSendInstructions(
    ...
    // for loops go BRRRRR
    for (let ix in instructions) {
      for (let addr in instructions[ix].keys) {
        let meta = instructions[ix].keys[addr];
        if (meta.isSigner) {
          for (let sgnr in this.signers) {
            let signer = this.signers[sgnr];
            if (signer.publicKey === meta.pubkey) {
              signers.push(signer);
            }
          }
        }
