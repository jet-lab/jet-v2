...
static TEST_CONTEXT: OnceCell<TestRuntime> = OnceCell::const_new();

pub async fn emulated_client() -> TestRuntime {
    TEST_CONTEXT
        .get_or_init(|| async {
            create_test_runtime![jet_control, jet_margin, jet_metadata, jet_bonds,]
        })
        .await
        .clone()
}
