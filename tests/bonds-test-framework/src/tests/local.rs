...
use crate::{emulated_client, BondsUser, TestManager, ONE_TOKEN, STARTING_TOKENS};

async fn _full_workflow(manager: Arc<TestManager>) -> Result<()> {
    let alice = BondsUser::new_funded(manager.clone()).await?;
    let bond_market = manager.load_manager().await?;

    const START_TICKETS: u64 = 1_000_000 * ONE_TOKEN;
    let exchange_amount = bond_market.convert_tickets(START_TICKETS)?;
    alice.convert_tokens(exchange_amount).await?;

    assert_eq!(alice.tickets().await?, START_TICKETS);
    assert_eq!(alice.tokens().await?, STARTING_TOKENS - exchange_amount);
    assert_eq!(
    ...
    let claim_amount = bond_market.convert_tickets(STAKE_AMOUNT)?;
    let ticket_seed = vec![];
