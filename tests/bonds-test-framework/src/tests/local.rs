...
use crate::{
    emulated_client, BondsUser, NoProxy, OrderAmount, Proxy, TestManager, STARTING_TOKENS,
};

async fn _full_workflow<P: Proxy>(manager: Arc<TestManager>) -> Result<()> {
...
#[serial_test::serial]
async fn emulated_full_direct() -> Result<()> {
    let client = emulated_client().await;
    let manager = TestManager::full(Arc::new(client)).await?;
...
#[tokio::test(flavor = "multi_thread")]
#[serial_test::serial]
async fn emulated_full_through_margin() -> Result<()> {
    let client = emulated_client().await;
    let manager = TestManager::full(Arc::new(client)).await?;
...
async fn margin() -> Result<()> {
    ...
    // create user
    let wallet = create_wallet(&manager.client, 100 * LAMPORTS_PER_SOL).await?;
