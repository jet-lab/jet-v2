...
async fn create_and_insert(
    ...
    let pool =
        OrcaWhirlpoolSwapPool::configure(rpc, &one, &two, 100_000_000 * ONE, 100_000_000 * ONE).await?;

    Ok((one, two, pool))
}

#[async_trait]
pub trait OrcaWhirlpoolSwapPoolConfig: Sized {
    async fn configure(
        rpc: &Arc<dyn SolanaRpcClient>,
        mint_a: &Pubkey,
        mint_b: &Pubkey,
...
#[async_trait]
impl OrcaWhirlpoolSwapPoolConfig for OrcaWhirlpoolSwapPool {
