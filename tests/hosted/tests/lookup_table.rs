...
async fn lookup_table() -> Result<(), anyhow::Error> {
    ...
    // // What happens when we extend the lookup table with the same accounts?
    // LookupTable::extend_lookup_table(&ctx.rpc, table, accounts).await?;
