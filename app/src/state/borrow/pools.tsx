...
export const FilteredPoolsList = selector<Pool[]>({
  ...
  get: ({ get }) => {
    ...
    const validPools = Object.values(list?.tokenPools).filter((pool: Pool) => !!pool.symbol);
...
export function PoolsWrapper(props: { children: JSX.Element }) {
  ...
  useEffect(() => {
    if (pools) {
      const currentPool = pools.tokenPools[currentPoolSymbol];
