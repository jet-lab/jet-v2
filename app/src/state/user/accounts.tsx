  ...
  useEffect(() => {
    for (const accountKey of Object.keys(accountNames)) {
      if (accountNames[accountKey] === currentAccountName) {
        setCurrentAccount(accounts.filter(account => account.address.toString() === accountKey)[0]);
      }
    }
...
export function useAccountFromName(accountName: string | undefined): MarginAccount | undefined {
  ...
  return useMemo(() => {
    ...
    for (const accountKey of Object.keys(accountNames)) {
      if (accountNames[accountKey] === accountName) {
        marginAccount = accounts.filter(account => account.address.toString() === accountKey)[0];
      }
    }
