...
type Subscription = { id: number; buffer: Buffer };
export function MarketWrapper(props: { children: JSX.Element[] }) {
  ...
  const [bidsSubscription, setBidsSubscription] = useState<Subscription>();
  const [asksSubscription, setAsksSubscription] = useState<Subscription>();
  ...
  useEffect(() => {
    async function currentMarketOrderbookSubscribe() {
      const market = Object.values(markets).filter(market => currentMarketPair === market.name)[0];
      if (market) {
        ...
        const bidsSub: Subscription = {} as Subscription;
        bidsSub.id = await getAccountInfoAndSubscribe(
          provider.connection,
          new PublicKey(market.marketConfig.bids),
          accountInfo => {
            if (accountInfo) {
        ...
        const asksSub: Subscription = {} as Subscription;
        asksSub.id = await getAccountInfoAndSubscribe(
          provider.connection,
          new PublicKey(market.marketConfig.asks),
          accountInfo => {
            if (accountInfo) {
