...
export function OrderEntry(): JSX.Element {
  ...
  const maxMarginBuy =
    currentAccount && currentMarket && currentPool?.symbol
      ? currentAccount.getMaxTradeAmounts(
          currentPool,
          currentAccount.poolPositions[currentPool.symbol].depositBalance,
          currentAccount.poolPositions[currentPool.symbol].loanBalance
        ).borrow
      : zeroTokenAmount;
  ...
  const marginDifferenceLong = getTokenAmountFromNumber(
    Math.max(orderSize.tokens - (quoteAssetPoolPosition?.depositBalance ?? zeroTokenAmount).tokens, 0),
  ...
  const marginDifferenceShort = baseAssetPoolPosition
    ? getTokenAmountFromNumber(
        Math.max(orderAmount.tokens - baseAssetPoolPosition.depositBalance.tokens, 0),
            ...
            <div className="order-entry-body-section-info-item flex align-center justify-between">
              ...
              <Paragraph type="danger">
                {currencyAbbrev(
                  ...
                  false,
                  undefined,
                  orderSide === 'sell' ? currentMarket?.baseDecimals : currentMarket?.quoteDecimals
                )}
