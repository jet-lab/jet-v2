...
import { CurrentPoolSymbol, PoolWithSymbol } from '../../../state/borrow/pools';

interface IPoolRow {
  pool: PoolWithSymbol;
}

export const PoolRow: React.FC<IPoolRow> = ({ pool }) => {
  const [currentPool, setCurrentPoolSymbol] = useRecoilState(CurrentPoolSymbol);
  ...
  return (
    <tr
      className={`ant-table-row ant-table-row-level-0 ${pool.symbol}-pools-table-row ${
        pool.symbol === currentPool ? 'active' : ''
      }`}
      ...
      <td className="ant-table-cell" style={{ textAlign: 'right' }}>
        {pool.depositNoteMetadata?.valueModifier ? (
          formatRate(pool.depositNoteMetadata.valueModifier.asNumber())
        ) : (
      ...
      <td className="ant-table-cell" style={{ textAlign: 'right' }}>
        {pool.borrowedTokens ? (
          `${currencyAbbrev(pool.borrowedTokens.tokens, fiatValues, pool.tokenPrice, 2)} ${
      ...
      <td className="ant-table-cell" style={{ textAlign: 'right' }}>
        {pool.vault ? (
          `${currencyAbbrev(pool.vault.tokens, fiatValues, pool.tokenPrice, 2)} ${!fiatValues ? pool.symbol : ''}`
        ) : (
          <Skeleton className="align-right" paragraph={false} active />
        )}
      </td>
