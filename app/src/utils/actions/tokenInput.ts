...
export function useTokenInputDisabledMessage(account?: MarginAccount): string {
  ...
  if (!pools || !currentPool?.symbol || !marginAccount) {
    return '';
  }
  ...
  if (currentAction === 'deposit') {
    ...
    if (poolPosition && !poolPosition.depositBalance.tokens) {
      ...
      // No wallet balance to swap
    } else if (walletTokens && !walletTokens.map[tokenSymbol].amount.tokens) {
      disabledMessage = dictionary.actions.swap.disabledMessages.noBalanceForSwap
        .replaceAll('{{ASSET}}', tokenSymbol)
        .replaceAll('{{WALLET_OR_ACCOUNT}}', dictionary.common.wallet.toLowerCase());
    }
    ...
    if (poolPosition && !poolPosition.depositBalance.tokens) {
      ...
      for (const accountKey of Object.keys(accountNames)) {
        if (accountKey === marginAccount.address.toString()) {
          accountName = accountNames[accountKey];
        }
      }
