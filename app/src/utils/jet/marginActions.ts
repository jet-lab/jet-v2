...
import { MarginAccount, Pool, PoolTokenChange, sleep, TokenAmount, TokenFaucet } from '@jet-lab/margin';
...
export function useMarginActions() {
  ...
  async function airdrop(pool: Pool): Promise<[string | undefined, ActionResponse]> {
    ...
    if (!pool?.symbol) {
      throw new Error('No Pool Symbol');
    }
  ...
  async function createAccount(accountName?: string): Promise<[string | undefined, ActionResponse]> {
    if (!programs || !pools || !currentPool?.symbol || !walletTokens || !wallet.publicKey) {
    ...
    try {
      ...
      const txId = await newMarginAccount.createAccount();
      if (accountName) {
        axios
  ...
  async function deposit(): Promise<[string | undefined, ActionResponse]> {
    if (!pools || !currentPool?.symbol || !walletTokens || !currentAccount || !accountPoolPosition) {
  ...
  async function withdraw(): Promise<[string | undefined, ActionResponse]> {
    if (!pools?.tokenPools || !currentPool?.symbol || !walletTokens || !currentAccount || !accountPoolPosition) {
      console.error('Accounts and/or pools not loaded');
      throw new Error();
    }
  ...
  // Repay
  async function repay(): Promise<[string | undefined, ActionResponse]> {
    if (!pools || !currentPool || !walletTokens || !currentAccount || !accountPoolPosition) {
      console.error('Accounts and/or pools not loaded');
      throw new Error();
    }
  ...
  async function transfer(
    ...
    if (!pools || !currentPool?.symbol || !currentAccount || !fromAccount.walletTokens || !toAccount.walletTokens) {
      console.error('Accounts and/or pools not loaded');
      throw new Error();
    }
